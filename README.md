...
After authentication, the application reports the WebLogic principal, remote user, authentication type, session, client/request context, role checks, and safely redacted request headers. It never renders authorization values, cookies, or complete assertions/tokens. If an assertion/JWT is deliberately propagated in a request header, it shows only a fingerprint and, for JWTs, decoded header and claims.
...
Set the `slo.redirect-uri` context parameter in `WEB-INF/web.xml` to that full external URL when the application is behind a reverse proxy or load balancer. Ensure the WebLogic installation includes the October 2023 PSU/SPB or a later maintenance level that provides SAML SLO support.
A
